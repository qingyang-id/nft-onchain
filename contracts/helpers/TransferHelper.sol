    ...
    internal
  {
    ...
    address conduit = address(
      uint160(
        uint256(
          keccak256(
            abi.encodePacked(bytes1(0xff), address(_CONDUIT_CONTROLLER), conduitKey, _CONDUIT_CREATION_CODE_HASH)
          )
        )
      )
    );
    ...
    try ConduitInterface(conduit).execute(conduitTransfers) returns (bytes4 conduitMagicValue) {
      ...
      if (
        data.length == 4 &&
        (customErrorSelector == InvalidItemType.selector || customErrorSelector == InvalidERC721TransferAmount.selector)
      ) {
